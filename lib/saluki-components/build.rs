...
fn main() {
    ...
    let schema_path = manifest_dir.join("etc/core_schema.yaml");
    let ignored_keys_path = manifest_dir.join("etc/ignored_keys.yaml");
    let out_dir = PathBuf::from(std::env::var("OUT_DIR").unwrap());
    let out_path = out_dir.join("schema.rs");

    // Only re-run when the schema source or this build script changes.
    println!("cargo:rerun-if-changed=etc/core_schema.yaml");
    println!("cargo:rerun-if-changed=etc/ignored_keys.yaml");
...
fn render(entries: &[SchemaEntry]) -> String {
    ...
    writeln!(out, "// @generated by build.rs from etc/core_schema.yaml — DO NOT EDIT").unwrap();
    writeln!(out).unwrap();
