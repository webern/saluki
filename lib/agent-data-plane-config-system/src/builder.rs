//! The Datadog witness implementation: [`SalukiConfigBuilder`].
//!
//! [`SalukiConfigBuilder`] implements [`DatadogConfigWitness`], so the generated `drive` calls one
...
//!
//! Most keys assign a single field directly. The four endpoint keys (`api_key`, `dd_url`, `site`,
//! `additional_endpoints`) are accumulated in scratch fields and assembled into
//! `shared.endpoints.endpoints` by [`SalukiConfigBuilder::finish`], because one model field is built
//! from several keys.
//!
//! Conversions that can fail (enum parsing, byte-size parsing, JSON structure parsing) record a
//! [`TranslateError`] via [`SalukiConfigBuilder::record_error`]; `drive` surfaces the first such
//! error after every key is consumed. On failure the field keeps its default value.
...
use agent_data_plane_config::shared::{EndpointConfig, ForwarderHttpProtocol};
use agent_data_plane_config::SalukiConfiguration;
use bytesize::ByteSize;
use datadog_agent_config::{DatadogConfigWitness, TranslateError};

/// Accumulates witnessed Datadog values into a [`SalukiConfiguration`].
///
/// Construct with [`SalukiConfigBuilder::default`], drive it with the generated `drive`, then call
/// [`SalukiConfigBuilder::finish`] to assemble the multi-key endpoint field and recover the model.
// TODO(visibility): add crate-boundary architectural guard when arch tests are wired up
#[derive(Debug, Default)]
pub(crate) struct SalukiConfigBuilder {
    config: SalukiConfiguration,
    endpoints: EndpointScratch,
    error: Option<TranslateError>,
}

/// Scratch storage for the four keys that together build `shared.endpoints.endpoints`.
#[derive(Debug, Default)]
...
impl EndpointScratch {
    /// Builds the resolved endpoint list from the accumulated scratch.
    ///
    /// The primary endpoint takes `dd_url` when set, else `site`, else `datadoghq.com`; each
    /// `additional_endpoints` entry becomes a further endpoint.
    fn into_endpoints(self) -> Vec<EndpointConfig> {
        let EndpointScratch {
            api_key,
            dd_url,
            site,
        ...
        endpoints.push(EndpointConfig {
            url,
            api_keys: non_empty(api_key).into_iter().collect(),
        });
        for (url, api_keys) in additional {
            endpoints.push(EndpointConfig { url, api_keys });
        }
        endpoints
    }
}

impl SalukiConfigBuilder {
    ...
    /// Assembles the multi-key endpoint field and returns the populated configuration.
    ///
    /// The primary endpoint is built from `api_key`, `dd_url`, and `site` (with `dd_url` taking
    /// precedence over `site`, which defaults to `datadoghq.com`); each `additional_endpoints` entry
    /// becomes a further endpoint. The result is assigned to `shared.endpoints.endpoints`.
    pub(crate) fn finish(mut self) -> SalukiConfiguration {
        self.config.shared.endpoints.endpoints = self.endpoints.into_endpoints();
        self.config
    }
}
...
/// Parses one mapper-profile JSON object into a [`MapperProfile`].
fn parse_mapper_profile(raw: serde_json::Value) -> Result<MapperProfile, String> {
...
/// Parses one tag-filterlist JSON object into a [`MetricTagFilterEntry`].
...
impl DatadogConfigWitness for SalukiConfigBuilder {
    ...
    fn consume_log_file_max_size(&mut self, value: String) {
        match value.parse::<ByteSize>() {
            ...
            Err(reason) => self.record_error(TranslateError::for_key("log_file_max_size", reason)),
        }
    }
    ...
    fn consume_dogstatsd_tag_cardinality(&mut self, value: String) {
        let cardinality = match value.to_ascii_lowercase().as_str() {
            ...
            other => {
                self.record_error(TranslateError::for_key(
    ...
    fn consume_dogstatsd_mapper_profiles(&mut self, value: Vec<serde_json::Value>) {
        ...
        for raw in value {
            match parse_mapper_profile(raw) {
                ...
                Err(reason) => {
                    self.record_error(TranslateError::for_key("dogstatsd_mapper_profiles", reason));
                    return;
                }
            }
        }
    ...
    fn consume_metric_tag_filterlist(&mut self, value: Vec<serde_json::Value>) {
        ...
        for raw in value {
            match parse_tag_filter_entry(raw) {
                ...
                Err(reason) => {
                    self.record_error(TranslateError::for_key("metric_tag_filterlist", reason));
                    return;
                }
            }
        }
    ...
    fn consume_dogstatsd_log_file_max_size(&mut self, value: String) {
        match value.parse::<ByteSize>() {
            ...
            Err(reason) => self.record_error(TranslateError::for_key("dogstatsd_log_file_max_size", reason)),
        }
    }
