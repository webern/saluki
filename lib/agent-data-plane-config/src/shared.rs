...
pub struct Endpoints {
    /// The resolved intake endpoints and the API keys used to authenticate to each.
...
/// One resolved endpoint: a URL and the API keys to ship to it. Assembled by the translator from
/// `api_key`, `dd_url`, `site`, and `additional_endpoints`.
#[derive(Clone, Debug, Default, Serialize)]
pub struct EndpointConfig {
    /// The endpoint URL that payloads are shipped to.
    pub url: String,

    /// API keys used to authenticate to this endpoint.
