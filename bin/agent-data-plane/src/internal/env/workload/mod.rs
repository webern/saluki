...
impl RemoteAgentWorkloadProvider {
    /// Create a new `RemoteAgentWorkloadProvider` from an established Datadog Agent IPC client.
    ///
    /// This path avoids opening another IPC connection for the remote tagger/workloadmeta collectors.
    pub async fn from_datadog_client(
        client: RemoteAgentClient, component_registry: ComponentRegistry, health_registry: &HealthRegistry,
        ...
        let string_interner_size_bytes = DEFAULT_STRING_INTERNER_SIZE_BYTES;
        ...
        let ra_tags_collector =
            build_collector("remote-agent-tags", health_registry, &mut collector_bounds, |health| {
                std::future::ready(Ok(RemoteAgentTaggerMetadataCollector::from_client(
                    client.clone(),
                    health,
        ...
        let ra_wmeta_collector =
            build_collector("remote-agent-wmeta", health_registry, &mut collector_bounds, |health| {
                std::future::ready(Ok(RemoteAgentWorkloadMetadataCollector::from_client(
                    client,
                    health,
                    string_interner,
                )))
        ...
        let on_demand_pid_resolver = OnDemandPIDResolver::noop();
