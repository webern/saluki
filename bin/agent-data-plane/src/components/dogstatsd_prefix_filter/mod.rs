...
use saluki_component_config::{
    DogStatsDPrefixFilterConfiguration as NativeDogStatsDPrefixFilterConfiguration, DynamicValue,
};
...
impl DogStatsDPrefixFilterConfiguration {
    /// Creates DogStatsD prefix/filter settings from native configuration.
    pub fn from_native(config: &NativeDogStatsDPrefixFilterConfiguration) -> Self {
        Self {
            metric_prefix: config.metric_prefix().to_string(),
            metric_prefix_blocklist: config.metric_prefix_blocklist().to_vec(),
            ...
            select! {
                ...
                maybe_new_filterlist_match_prefix = self.metric_filterlist_match_prefix.changed() => {
                    if let Some(new_match_prefix) = maybe_new_filterlist_match_prefix {
                        debug!(new_match_prefix, key = METRIC_FILTERLIST_MATCH_PREFIX_CONFIG_KEY, "Updated metric filterlist match prefix.");
                ...
                maybe_new_blocklist_match_prefix = self.metric_blocklist_match_prefix.changed() => {
                    if let Some(new_match_prefix) = maybe_new_blocklist_match_prefix {
                        debug!(new_match_prefix, key = STATSD_METRIC_BLOCKLIST_MATCH_PREFIX_CONFIG_KEY, "Updated metric blocklist match prefix.");
...
mod tests {
    ...
    fn test_metric_blocklist_dynamic_update() {
        ...
        filter.update_metric_blocklist(vec!["foo".to_string()]);
        ...
        // Apply the dynamic update to the filter under test.
        filter.update_metric_filterlist(vec!["baz".to_string()]);
    ...
    fn test_metric_filterlist_match_prefix_dynamic_update_is_applied() {
        ...
        // Apply the dynamic update to the filter under test.
        filter.update_metric_filterlist_match_prefix(true);
