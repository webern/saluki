...
fn main() {
    ...
    // Run the expensive prune + typify pass once and share the resulting AST with both generators
    // so the witness trait's method types cannot drift from the generated `DatadogConfiguration`.
    let typified = datadog_config_gen::typify_schema(&overlay, &schema_path);
    datadog_config_gen::generate(&typified, &manifest_dir);
    witness_gen::generate(&overlay, &typified, &manifest_dir);
}
