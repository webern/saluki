//! Datadog Agent local-source key aliases and environment remappings.
...
/// A Figment provider that remaps canonical environment variable names to Datadog config keys.
