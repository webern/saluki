//! The `run` subcommand definition and its runtime orchestration.
//!
//! The command builds the data topology entirely from the native [`SalukiConfiguration`] (see
//! `create_topology`) and assembles the supervision tree. It holds no `GenericConfiguration`: the
//! resolved configuration comes from the configuration system's startup seam
//! ([`ConfigurationSystem::start`]), which runs the raw loading entirely and returns only typed
//! outputs.
...
use crate::internal::{create_internal_supervisor, DogStatsDControlSurface, TopologyControlSurfaces};
...
///
/// Runs the configuration system's startup seam from `inputs`/`service_names` to obtain the
/// ADP-native [`SalukiConfiguration`] plus the typed capabilities (config snapshot, IPC certificate
/// path, optional Datadog Agent connection and dynamic-configuration handles). This builds the data
/// topology and assembles the supervision tree; it does not touch `GenericConfiguration`.
pub async fn handle_run_command(
    started: Instant, inputs: BootstrapInputs, service_names: Vec<String>, bootstrap_guard: &mut BootstrapGuard,
    ...
    // Run the configuration system's startup seam. It owns the raw loading entirely and returns only
    // typed outputs: the ADP-native `SalukiConfiguration`, a config snapshot, the IPC certificate
    // path, and the (optional) Datadog Agent connection and dynamic-configuration handles.
    let started_config = ConfigurationSystem::new(inputs, service_names)
        .start()
        .await
        .error_context("Failed to start the ADP configuration system.")?;

    if !started_config.saluki().data_plane.enabled() {
        info!("Agent Data Plane is not enabled. Exiting.");
        return Ok(());
    }

    // Reload logging to reflect the authoritative (native) logging configuration.
    match logging_configuration_from_native(&started_config.saluki().logging) {
        Ok(logging_config) => {
    ...
    let parts = started_config.into_parts();
    ...
    let saluki_config: SalukiConfiguration = parts.saluki;
    let config_snapshot: serde_json::Value = parts.config_snapshot;
    let ipc_cert_path: PathBuf = parts.ipc_cert_path;
    ...
    let (env_provider, maybe_env_supervisor) = ADPEnvironmentProvider::from_saluki(
        &saluki_config,
        connection.as_ref().map(|c| c.client()),
        &component_registry,
        &health_registry,
    )
    .await?;
    ...
    //
    // The Remote Agent gRPC services are built from the connection's session (the service split); the
    // control plane is fed the config snapshot and IPC certificate path. No raw configuration is consumed.
    let services = match &connection {
        Some(connection) => Some(RemoteAgentServices::from_session(connection.session_id()).await),
        None => None,
    };
    let log_level_handle = dynamic_handles.as_ref().map(|h| h.log_level.clone());
    let mut internal_supervisor = create_internal_supervisor(
        config_snapshot.clone(),
        ipc_cert_path.clone(),
        &saluki_config.data_plane,
        &component_registry,
