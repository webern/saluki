...
mod tests {
    use super::*;
    use crate::config_registry::Severity;
    ...
    fn none_default_null_is_default() {
        let c = classifier();
        // dogstatsd_buffer_count is a SALUKI_ENTRY with no schema default
        let result = c.classify("dogstatsd_buffer_count", &Value::Null).unwrap();
        assert!(result.is_default);
    }
    ...
    fn none_default_empty_string_is_default() {
        let c = classifier();
        let result = c.classify("dogstatsd_buffer_count", &Value::String("".into())).unwrap();
        assert!(result.is_default);
    }
    ...
    fn none_default_non_empty_is_not_default() {
        let c = classifier();
        let result = c
            .classify("dogstatsd_buffer_count", &Value::String("something".into()))
            .unwrap();
        assert!(!result.is_default);
    }
