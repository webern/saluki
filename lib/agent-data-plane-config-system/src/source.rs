...
//!
//! Saluki-schema-only keys are those the vendored Datadog Agent schema does not publish. Their
//! authority is Saluki's own source (`SALUKI_*` / `saluki.yaml`), so these structs derive
