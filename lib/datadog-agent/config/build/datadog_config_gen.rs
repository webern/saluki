...
/// The shared product of the prune + typify pass.
///
/// Both code generators read from the same `typify` output so they cannot disagree about a leaf's
/// Rust type: `datadog_config_gen` renders it into the checked-in `datadog_configuration.rs`, and
/// `witness_gen` walks the same AST to read each leaf's type token verbatim for the witness trait.
pub struct TypifiedSchema {
    /// The dotted paths the overlay marks `support: full` / `support: partial`.
    pub supported: HashSet<String>,
    /// The `syn::File` `typify` produced from the pruned schema, before doc/path normalization.
    pub file: syn::File,
}

/// Run the prune + typify pass once and return its product for both code generators to share.
///
/// This is the expensive step (one `typify` run over the pruned schema), so it is performed a
/// single time per build and the resulting AST is reused.
pub fn typify_schema(overlay: &SchemaOverlay, schema_path: &Path) -> TypifiedSchema {
    ...
    let file = typify_file(pruned_schema);

    TypifiedSchema { supported, file }
}

pub fn generate(typified: &TypifiedSchema, manifest_dir: &Path) {
    let body = render(typified.file.clone());
...
/// Run typify over the pruned schema and parse its output into a `syn::File`.
fn typify_file(pruned_schema: Value) -> syn::File {
...
/// Normalize and pretty-print the typify output into the checked-in module body.
fn render(mut file: syn::File) -> String {
